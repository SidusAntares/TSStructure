...
class PseFreDNLTae(nn.Module):
    ...
    def prepare_temporal_features(self, spatial_feats, positions):
        ...
        reconstructed_diagnostics = dict(self.fourier_synthesizer.last_diagnostics)
        trend_complex = self.fourier_synthesizer.synthesize_complex(
            trend_coeffs,
            positions,
        )
        trend_diagnostics = dict(self.fourier_synthesizer.last_diagnostics)
        seasonal_complex = self.fourier_synthesizer.synthesize_complex(
            seasonal_coeffs,
            positions,
        )
        seasonal_diagnostics = dict(self.fourier_synthesizer.last_diagnostics)
        ...
        diagnostics.update(
            {
                "additivity_error": float(additivity_error.detach().cpu()),
                "reconstruction_error": float(reconstruction_error.detach().cpu()),
                "imaginary_residual": reconstructed_diagnostics[
                    "imaginary_residual"
                ],
                "synthesis_time": (
                    reconstructed_diagnostics["synthesis_time"]
                    + trend_diagnostics["synthesis_time"]
                    + seasonal_diagnostics["synthesis_time"]
                ),
            }
        )
