...
SEED=1
RUN_ROOT="timematch_shape13_morphonly_3tasks_seed${SEED}"
...
print_task() {
    ...
    local weights="$4"
    local experiment="timematch_shape13_morphonly_${source_alias}_${target_alias}_seed${SEED}"
...
launch "$GPU0" DK1 "$DK1" FR1 "$FR1" "$DK1_WEIGHTS"
launch "$GPU1" FR1 "$FR1" FR2 "$FR2" "$FR1_WEIGHTS"
launch "$GPU2" FR2 "$FR2" AT1 "$AT1" "$FR2_WEIGHTS"

echo "Three TimeMatch + Shape13 MorphOnly ablations launched; GPU3 is unused."
