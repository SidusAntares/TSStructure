...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/mtkd_tq_mid_12tasks}"
TENSORBOARD_ROOT="${TENSORBOARD_ROOT:-runs/mtkd_tq_mid_12tasks}"
LOG_ROOT="${LOG_ROOT:-logs/mtkd_tq_mid_12tasks}"
DRY_RUN="${DRY_RUN:-0}"
SEED=1
NUM_FOLDS=1

GPU_IDS=(0 1 2 3)
SOURCE_DOMAINS=(AT1 DK1 FR1 FR2)
...
source_experiment_name() {
    printf 'mtkd_tq_mid_%s_source_seed%s' "$1" "$SEED"
}

adaptation_experiment_name() {
    printf 'mtkd_tq_mid_%s_%s_seed%s' "$1" "$2" "$SEED"
}
...
run_dry_run() {
    ...
    printf '[DRY-RUN] 4 source commands + 12 TimeMatch DA commands; no training launched.\n'
    printf '\nDRY-RUN SUMMARY\n'
    printf 'source commands:       4\n'
    printf 'TimeMatch commands:    12\n'
    printf 'unique DA tasks:       12\n'
    printf 'GPU0:                  AT1\n'
    printf 'GPU1:                  DK1\n'
    printf 'GPU2:                  FR1\n'
    printf 'GPU3:                  FR2\n'
    printf 'model:                 psemtkdtqmidltae\n'
    printf 'seed:                  %s\n' "$SEED"
    printf 'num_folds:             %s\n' "$NUM_FOLDS"
...
launch_workers() {
    ...
    local script_path="$PROJECT_DIR/scripts/run_mtkd_tq_mid_12tasks_4gpu.sh"
    ...
    for index in "${!SOURCE_DOMAINS[@]}"; do
        gpu_id="${GPU_IDS[$index]}"
        source="${SOURCE_DOMAINS[$index]}"
        worker_log="$run_root/gpu${gpu_id}_${source,,}.log"
