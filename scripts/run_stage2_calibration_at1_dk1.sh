...
RUN_GROUP="${RUN_GROUP:-stage2_calibration_roundc_at1_dk1}"
...
export CODE_VERSION="structure_da_v3_roundc_calibration_a"

echo "STAGE2_CALIBRATION_START|checkpoint=${STAGE1_CHECKPOINT}|config=${STAGE2_CONFIG}|gpu=${CUDA_DEVICE}"
run_training \
    "${SOURCE_DOMAIN}" \
    "${TARGET_DOMAIN}" \
    "${SEED}" \
