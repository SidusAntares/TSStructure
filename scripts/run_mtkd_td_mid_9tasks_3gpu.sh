...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/mtkd_td_mid_9tasks}"
TENSORBOARD_ROOT="${TENSORBOARD_ROOT:-runs/mtkd_td_mid_9tasks}"
LOG_ROOT="${LOG_ROOT:-logs/mtkd_td_mid_9tasks}"
DRY_RUN="${DRY_RUN:-0}"
SEED=1
NUM_FOLDS=1
...
source_experiment_name() {
    printf 'mtkd_td_mid_%s_source_seed%s' "$1" "$SEED"
}

adaptation_experiment_name() {
    printf 'mtkd_td_mid_%s_%s_seed%s' "$1" "$2" "$SEED"
}
...
launch_workers() {
    ...
    local script_path="$PROJECT_DIR/scripts/run_mtkd_td_mid_9tasks_3gpu.sh"
