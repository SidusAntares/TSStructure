...
EXP_ROOT="${EXP_ROOT:-outputs/structure_proto_v2clean_4tasks_seed1}"
LOG_ROOT="${LOG_ROOT:-logs/structure_proto_v2clean_4tasks_seed1}"
RUN_ROOT="${RUN_ROOT:-runs/structure_proto_v2clean_4tasks_seed1}"
...
run_task() {
  ...
  : > "$log_file"
  echo "[V2-CLEAN SOURCE START] $src_name GPU$gpu" | tee -a "$log_file"
  ...
  if [[ ! -f "$source_weights/fold_0/model.pt" ]]; then
    echo "ERROR: V2-Clean source checkpoint not found: $source_weights/fold_0/model.pt" | tee -a "$log_file"
    return 1
  fi

  echo "[V2-CLEAN UDA START] $src_name -> $tgt_name GPU$gpu" | tee -a "$log_file"
  CUDA_VISIBLE_DEVICES="$gpu" python -u train.py \
    -e "${task}_seed1" \
