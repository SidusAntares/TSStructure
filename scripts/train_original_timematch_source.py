...
def main(config):
    ...
    for fold_num, splits in enumerate(folds):
        ...
        source_val_loader, _ = create_evaluation_loaders(config.source, splits, config, sample_pixels_val)
        _, target_test_loader = create_evaluation_loaders(config.target, splits, config, sample_pixels_val)
        ...
        if not config.eval:
            ...
            train_supervised(model, config, writer, splits, source_val_loader, device, best_model_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='pseltae', choices=['pseltae'])
