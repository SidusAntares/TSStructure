...
EXP_ROOT="${EXP_ROOT:-outputs/structure_proto_kmeans_init_4tasks_seed1}"
LOG_ROOT="${LOG_ROOT:-logs/structure_proto_kmeans_init_4tasks_seed1}"
...
mkdir -p "$LOG_ROOT" "$EXP_ROOT/source" "$EXP_ROOT/uda" "$EXP_ROOT/runs"
...
run_task() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u train.py \
    -e "$source_experiment" \
    --data_root "$DATA_ROOT" \
    --source "$src_data" --target "$src_data" \
    ...
    --shapelet-count 16 --shapelet-beta 5 --shape-resample-length 16 --shapelet-init kmeans \
    ...
    --tensorboard_log_dir "$EXP_ROOT/runs/source_${src_name}_seed1" \
    2>&1 | tee -a "$log_file"
  ...
  {
    echo "============================================================"
    echo "UDA $src_name -> $tgt_name"
    echo "SOURCE CHECKPOINT=$source_weights/fold_0/model.pt"
    echo "============================================================"
  } | tee -a "$log_file"
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u train.py \
    ...
    --tensorboard_log_dir "$EXP_ROOT/runs/${task}_seed1" \
    timematch \
