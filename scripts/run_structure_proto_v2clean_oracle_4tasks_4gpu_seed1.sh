...
EXP_ROOT="${EXP_ROOT:-outputs/structure_proto_v2clean_oracle_4tasks_seed1}"
LOG_ROOT="${LOG_ROOT:-logs/structure_proto_v2clean_oracle_4tasks_seed1}"
RUN_ROOT="${RUN_ROOT:-runs/structure_proto_v2clean_oracle_4tasks_seed1}"
...
run_task() {
  ...
  : > "$log_file"
  echo "[V2-CLEAN ORACLE UDA START] $src_name -> $tgt_name GPU$gpu" | tee -a "$log_file"
  CUDA_VISIBLE_DEVICES="$gpu" python -u train.py \
    -e "v2clean_oracle_${task}_seed1" \
    ...
    --output_student true --oracle-pseudo-labels true \
    2>&1 | tee -a "$log_file"
