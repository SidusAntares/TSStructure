...
SOURCE_ROOT="${SOURCE_ROOT:-outputs/structure_proto_v4_4tasks_seed1/source}"
EXP_ROOT="${EXP_ROOT:-outputs/structure_proto_v5_4tasks_seed1}"
LOG_ROOT="${LOG_ROOT:-logs/structure_proto_v5_4tasks_seed1}"
RUN_ROOT="${RUN_ROOT:-runs/structure_proto_v5_4tasks_seed1}"
...
run_task() {
  ...
  if [[ ! -f "$checkpoint" ]]; then
    echo "ERROR: V4 source checkpoint not found: $checkpoint" | tee "$log_file"
    return 1
  fi

  echo "[UDA START] $src_name -> $tgt_name GPU$gpu" | tee "$log_file"
