...
EXP_ROOT="${EXP_ROOT:-outputs/structure_proto_v4_4tasks_seed1}"
LOG_ROOT="${LOG_ROOT:-logs/structure_proto_v4_4tasks_seed1}"
RUN_ROOT="${RUN_ROOT:-runs/structure_proto_v4_4tasks_seed1}"
...
run_task() {
  ...
  if [[ ! -f "$source_weights/fold_0/model.pt" ]]; then
    echo "ERROR: V4 source checkpoint not found: $source_weights/fold_0/model.pt" | tee -a "$log_file"
    return 1
  fi
