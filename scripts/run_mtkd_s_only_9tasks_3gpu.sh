...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/mtkd_s_only_9tasks}"
TENSORBOARD_ROOT="${TENSORBOARD_ROOT:-runs/mtkd_s_only_9tasks}"
LOG_ROOT="${LOG_ROOT:-logs/mtkd_s_only_9tasks}"
DRY_RUN="${DRY_RUN:-0}"
SEED=1
NUM_FOLDS=1
...
source_experiment_name() {
    printf 'mtkd_s_only_%s_source_seed%s' "$1" "$SEED"
}

adaptation_experiment_name() {
    printf 'mtkd_s_only_%s_%s_seed%s' "$1" "$2" "$SEED"
}
...
launch_workers() {
    ...
    local script_path="$PROJECT_DIR/scripts/run_mtkd_s_only_9tasks_3gpu.sh"
