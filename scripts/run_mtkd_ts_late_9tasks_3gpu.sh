...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/mtkd_ts_late_9tasks}"
TENSORBOARD_ROOT="${TENSORBOARD_ROOT:-runs/mtkd_ts_late_9tasks}"
LOG_ROOT="${LOG_ROOT:-logs/mtkd_ts_late_9tasks_3gpu}"
DRY_RUN="${DRY_RUN:-0}"
SEED=1
NUM_FOLDS=1

GPU_IDS=(1 2 3)
SOURCE_DOMAINS=(AT1 FR1 FR2)
...
source_experiment_name() {
    printf 'mtkd_ts_late_%s_source_seed%s' "$1" "$SEED"
}

adaptation_experiment_name() {
    printf 'mtkd_ts_late_%s_%s_seed%s' "$1" "$2" "$SEED"
}
...
run_dry_run() {
    ...
    printf '[DRY-RUN] 3 source commands + 9 TimeMatch DA commands; no training launched.\n'
}

launch_workers() {
    local run_root="$1"
    local launcher_log="$run_root/launcher.log"
    local script_path="$PROJECT_DIR/scripts/run_mtkd_ts_late_9tasks_3gpu.sh"
