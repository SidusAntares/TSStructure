...
SOURCE_ROOT="${SOURCE_ROOT:-outputs/structure_proto_v2_4tasks_seed1/source}"
EXP_ROOT="${EXP_ROOT:-outputs/structure_proto_v3_4tasks_seed1}"
LOG_ROOT="${LOG_ROOT:-logs/structure_proto_v3_4tasks_seed1}"
RUN_ROOT="${RUN_ROOT:-runs/structure_proto_v3_4tasks_seed1}"
...
mkdir -p "$LOG_ROOT" "$EXP_ROOT/uda" "$RUN_ROOT"
...
run_task() {
  ...
  if [[ ! -f "$checkpoint" ]]; then
    echo "ERROR: V2 source checkpoint not found: $checkpoint" | tee -a "$log_file"
    return 1
  fi
