...
def test_multiscale_windows_have_deterministic_order_and_mask():
    curve = torch.randn(2, 64, 4)
    windows, mask, scales = MultiScaleWindowExtractor((16, 32), 8)(curve)
    assert windows.shape == (2, 12, 32, 4)
    assert mask.shape == (2, 12, 32)
    assert scales.tolist() == [16] * 7 + [32] * 5
    assert mask[:, :7].sum(dim=-1).eq(16).all()
    assert mask[:, 7:].sum(dim=-1).eq(32).all()
...
def test_full_model_returns_all_training_intermediates_without_second_forward():
    ...
    assert model.structure_branch.token_generator.raw_encoder.network[0].weight.grad is not None
